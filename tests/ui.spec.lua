...
test("catalog cards normalize optional images and adapt their grid", function()
	...
	local card = library:AddScriptCard({ Name = "Sample", Description = "A custom script", Image = { AssetId = "12345", ScaleType = "Fit" } }, function() end)
	assert(card.Image.Visible and card.Image.ScaleType == Mock.Env.Enum.ScaleType.Crop, "covers must fill their available area")
	...
	assert(not card.Cover:FindFirstChild("CardHoverHint") and not card.Cover:FindFirstChild("CardHoverWash"))
	Mock.Resize(1280, 800)
...
test("controls use border strokes, solid navigation badges and avatar thumbnails", function()
	...
	assert(homeIcon and homeIcon:IsA("ImageLabel") and homeIcon.Image == "rbxassetid://7733960981")
	local modulesIcon = window._tabsByName.Modules.IconLabel:FindFirstChild("NavigationBadge_modules")
	assert(modulesIcon and modulesIcon:FindFirstChild("ModuleBar"), "modules should use solid module bars")
...
test("all theme previews apply complete palettes with distinct hover and selection", function()
	...
	assert(UI.Themes.Black.Background ~= UI.Themes.Midnight.Background)
	assert(UI.Themes.Frost.Panel ~= UI.Themes.Rose.Panel)
...
test("favorites create synchronized shortcuts and persist in UI state", function()
	...
	local outline = source.FavoriteIcon:FindFirstChild("HeartOutline")
	assert(not fill.Visible and outline.Visible, "unsaved favorites must have an outline-only heart")
	...
	assert(fill.Visible and not outline.Visible, "saved favorites must fill the heart")
	window:SetTheme("Rose")
	...
	assert(not fill.Visible and outline.Visible)
	window:SetTheme("Black")
