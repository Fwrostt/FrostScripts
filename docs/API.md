...
Use `window:AddTab(name, icon, subtitle)` then `tab:AddModule(options)`. Modules support `AddToggle`, `AddSlider`, `AddDropdown`, `AddMultiDropdown`, `AddNumberInput`, `AddButton`, `AddParagraph`, `AddColorPicker`, and `AddKeybind`. Shortcuts belong inside their owning modules. Pass a feature as `options.Feature` to bind its UI state.
